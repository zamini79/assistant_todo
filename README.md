...
전략 Assistant는 주기적으로 담당자가 바뀌고 수신자는 지시사항마다 다르므로,
둘 다 코드가 아니라 운영 중에 바꿀 수 있게 뒀다.

- **전략 Assistant** — 이름·이메일. 등록한 주소는 Remind 발송 시 **참조(CC)** 로 함께 받고,
  메일 서명에도 이름이 들어간다. 추후 사내 시스템과 연동해 이름만 넣으면 채워질 자리다.
- **메일 수신자** — 마스터 목록(이름·이메일·조직). 같은 주소는 대소문자 무시하고 한 번만
  등록된다(DB 유니크 인덱스). 삭제 시 몇 개의 지시사항에 연결돼 있는지 알려준다.
- **메일 발송 상태** — 읽기 전용. SMTP 비밀번호는 환경변수에만 두고 화면에 노출하지 않는다.
...
담당자(`assignee_email`)는 항상 포함되고, 지시사항마다 지정한 추가 수신자가 더해진다.
추가 수신자는 등록/수정 모달에서 설정의 마스터 목록 중 골라 지정한다(`todo_recipients`).

담당자가 수신자 목록에도 등록돼 있으면 중복을 제거해 한 번만 보낸다 —
대소문자만 다른 주소도 같은 사람으로 본다(`resolveRecipients`).
