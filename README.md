...
Remind 큐에서 항목을 고르고 **선택 발송**을 누르면 담당자에게 메일이 나간다.

### 설정 — Gmail로 시작해 사내 메일서버로 간다

벤더 중립 SMTP 설정이라 **전환은 env 교체만으로 끝난다.** 코드는 그대로다.

| | Gmail (현재) | 사내 메일서버 (추후) |
| --- | --- | --- |
| `SMTP_HOST` | `smtp.gmail.com` | `mail.사내도메인` |
| `SMTP_PORT` | `465` | `587` 등 |
| `SMTP_SECURE` | 생략 (465면 자동 TLS) | `false` (STARTTLS) |

Gmail은 2단계 인증을 켠 뒤 [앱 비밀번호](https://myaccount.google.com/apppasswords)
16자를 발급받아 `SMTP_PASS`에 넣는다. **계정 비밀번호로는 로그인되지 않는다.**

### 수신자마다 따로 보낸다

한 통에 여러 수신자를 넣지 않는다. 사내 게이트웨이가 외부 발신자의 다수 수신자 메일을
대량메일로 보고 일부에게만 도달하는 일이 실제로 있었다(사내 IR 클리핑 프로젝트).
개별 발송은 수신자 목록을 서로에게 감추고, 한 건이 실패해도 나머지는 계속 나간다.

실패한 건은 `wait`로 남아 다시 시도할 수 있고, 성공·실패 모두 `remind_logs`에 기록된다.

### 발송 경로는 두 가지

- **브리핑 → Remind 큐**: 여러 건을 골라 한 번에 보낸다. **템플릿** 버튼으로
  실제 나갈 메일을 미리 볼 수 있다.
- **표 → Remind 열**: 눈앞의 한 건을 바로 보낸다(종이비행기 아이콘).

발송 이력은 표에서 지시사항을 펼치면 **Remind 발송 이력**으로 보인다 —
언제·누구에게·성공했는지. 뱃지는 현재 상태만, 이력은 경과를 답한다.

### 주소가 없으면 발송 대상에서 빠진다

`assignee_email`은 선택 입력이다(등록 모달에서 직접 입력). 비어 있으면 큐에
"이메일 미등록"으로 표시되고 발송에서 제외된다 — 주소를 모른다고 지시사항 등록까지
막지는 않는다.
...
- **사내 메일서버 연동** — 현재는 Gmail SMTP로 실제 발송한다. 사내 서버로 옮길 때는
  `SMTP_*` env만 교체하면 되고 코드는 손대지 않는다 (위 "Remind 메일 발송" 참고).
- **Remind 스케줄러** — 발송은 수동(선택 발송)이다. 자동 발송은 아직 없다.
...
폼 검증, 진행 이력(최신순 접기·현재 상태 갱신·삭제 롤백), 리포지토리 계약,
메일 설정 파싱(포트별 TLS 판정)과 Remind 템플릿(HTML 이스케이프·지연 분기)을 덮는다.
UI 렌더링 테스트는 아직 없다.
